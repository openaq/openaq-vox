...
function getWelcomeResponse (callback) {
  ...
  const speechOutput = 'Welcome to the OpenAQ skill, you can say things like; "give me an overview" or; "how many stations are over WHO guidelines" or; "give me more details". All data comes from the OpenAQ platform and is aggregated from air quality sources around the world.';
...
function getAirQuality (overview, intent, session, callback) {
  const repromptText = null;
  const sessionAttributes = {};
  ...
  https.get(`${process.env.API_URL}v1/latest?parameter=pm25&limit=2000`, (res) => {
    res.on('data', (d) => {
      data += d;
    });
    ...
    res.on('end', () => {
      ...
      // Grab the recent measurements
      let locations = [];
      let numRecent = 0;
      let numOver = 0;
      json.results.forEach((l) => {
        if ((new Date() - new Date(l.measurements[0].lastUpdated)) / (1000 * 60 * 60) <= recentHours) {
          numRecent++;
          if (l.measurements[0].value >= 25) {
            numOver++;
          }
          // Only save if >= 0
          if (l.measurements[0].value > 0) {
            locations.push(l);
          }
        }
      });
      ...
      // Grab highest and lowest countries
      let highest = [];
      let lowest = [];
      while (highest.length < 5) {
        const l = locations.shift();
        if (highest.indexOf(l.country) === -1) {
          highest.push(l.country);
        }
      }
      while (lowest.length < 5) {
        const l = locations.pop();
        if (lowest.indexOf(l.country) === -1) {
          lowest.push(l.country);
        }
      }

      speechOutput = `OpenAQ has data from ${String(numRecent).replace(/(.)(?=(\d{3})+$)/g, '$1,')} stations reporting worldwide in the last ${recentHours} hours. ${((numOver / numRecent) * 100).toFixed()} percent of stations are reporting values over the WHO guidelines if persistent for 24 hours.`;

      // Add more info if it's an overview
      if (overview) {
        speechOutput += ` The countries containing stations with the highest PM2.5 values are ${getCountryName(highest[0])}, ${getCountryName(highest[1])}, ${getCountryName(highest[2])}, ${getCountryName(highest[3])} and ${getCountryName(highest[4])}. The countries reporting the lowest values are ${getCountryName(lowest[0])}, ${getCountryName(lowest[1])}, ${getCountryName(lowest[2])}, ${getCountryName(lowest[3])} and ${getCountryName(lowest[4])}.`;
      }
...
function getDisclaimer (intent, session, callback) {
  const repromptText = null;
  const sessionAttributes = {};
  let shouldEndSession = true;

  let speechOutput = "So we've got a lot of data, but, in fact, we do not measure any of it ourselves, we aggregate it from public real-time data sources provided by official, usually government-level, organizations. They do the hard work of measuring these data and publicly sharing them, and we do the work of making them more universally accessible to both humans and machines.";
...
function getCountryName (code) {
  for (let i = 0; i < countryCodes.length; i++) {
    if (countryCodes[i]['alpha-2'] === code) {
...
function onIntent (intentRequest, session, callback) {
  ...
  if (intentName === 'GetAirQualityWHO') {
    getAirQuality(false, intent, session, callback);
  } else if (intentName === 'GetAirQualityOverview') {
    getAirQuality(true, intent, session, callback);
  } else if (intentName === 'GetDisclaimer') {
    getDisclaimer(intent, session, callback);
  } else if (intentName === 'AMAZON.HelpIntent' || intentName === 'GetHelp') {
    getWelcomeResponse(callback);
